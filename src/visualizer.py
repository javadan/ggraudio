...
class Spectrum_Visualizer:
    ...
    def __init__(self, ear):
        ...
        self.chicken_high_freq = 5200
        self.lower_bin = 0
        self.upper_bin = self.ear.n_frequency_bins
    ...
    def start(self):
        ...
        pygame.init()
        self.screen = pygame.display.set_mode((self.WIDTH, self.EXTRA_HEIGHT_FOR_TOTALS_GRAPH)) #GGR change
    ...
    def update(self):
        ...
        if self.plot_audio_history:
            ...
            #GGR move 
            new_w, new_h = int((2+self.shrink_f)/3*self.WIDTH), int(self.HEIGHT + self.shrink_f*self.HEIGHT)
        ...
        #GGR stuff
        for x, y in self.bin_lines:
            pygame.draw.line(self.screen, (255,255,255), (x, 0), (x, self.HEIGHT - self.textRect.height), 3)

        self.plot_volume_graph()
    ...
    def plot_volume_graph(self):
        ...
        total = sum(chicken_range)

        self.totals.append(total)
        
        for i, total in enumerate(self.totals):
            x = self.WIDTH - len(self.totals) + i
            y = self.HEIGHT - int(total)
    ...
    def plot_bars(self):
        ...
        if self.add_fast_bars:
            for i, fast_bar in enumerate(self.fast_bars):
                fast_bar[1] = self.HEIGHT  #GGR move
        ...
        if self.add_slow_bars:
            for i, slow_bar in enumerate(self.slow_bars):
                #GGR change: slow follows fast, but we move it up, so bug fix for t=0
                if (slow_bar[1] < self.HEIGHT):
                    slow_bar[1] = slow_bar[1] + self.HEIGHT    
